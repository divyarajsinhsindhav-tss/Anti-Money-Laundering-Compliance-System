...
BEGIN
    ...
    WITH evaluated_data AS (
        -- STEP 1: The Evaluator (Row-by-Row Analysis)
        SELECT
            ...
            -- BUCKET 1: CRITICAL ERRORS (AML & Database Integrity)
            ARRAY_REMOVE(ARRAY[
            ...
            -- BUCKET 2: WARNING ERRORS (Data Typing Typos)
            ARRAY_REMOVE(ARRAY[
                CASE WHEN s.dob IS NOT NULL AND TRIM(s.dob) <> '' AND pg_input_is_valid(s.dob, 'date') = false THEN 'INVALID_DOB_FORMAT' END,
                CASE WHEN s.income IS NOT NULL AND TRIM(s.income) <> '' AND pg_input_is_valid(s.income, 'numeric') = false THEN 'INVALID_INCOME_FORMAT' END,
   ...
   upsert_customers AS (
        -- STEP 4: The Parent UPSERT
        INSERT INTO customer (
            customer_id, tenant_id, cif, first_name, middle_name, last_name, dob, income, 
            created_at, updated_at, is_deleted -- THE FIX: Added is_deleted
        )
        SELECT 
            gen_random_uuid(), p_tenant_id, cif, first_name, middle_name, last_name, 
            dob::DATE, income::NUMERIC(18,2),
            CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, false -- THE FIX: Inject boolean false
...
-- STEP 5: The Child UPSERT
    INSERT INTO account (
        account_id, tenant_id, customer_id, account_number, account_type, opened_at,
        created_at, updated_at, is_deleted -- THE FIX: Added is_deleted
    )
    SELECT 
        gen_random_uuid(), p_tenant_id, u.customer_id, e.account_number, e.account_type, e.opened_at::TIMESTAMP,
        CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, false -- THE FIX: Inject boolean false
