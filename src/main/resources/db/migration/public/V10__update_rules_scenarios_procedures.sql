-- 1. Remove columns that are no longer in the entities
...
ALTER TABLE public.scenarios
    DROP COLUMN IF EXISTS created_by;
ALTER TABLE public.scenarios
    RENAME COLUMN name TO scenario_name;

-- 2. Drop triggers for code generation since they will be entered manually
DROP TRIGGER IF EXISTS trg_rule_code_insert ON public.rules;
DROP TRIGGER IF EXISTS trg_scenario_code_insert ON public.scenarios;
DROP FUNCTION IF EXISTS trg_set_rule_code();
DROP FUNCTION IF EXISTS trg_set_scenario_code();

-- 3. Update the stored procedures to match the new CustomerError and TransactionError
-- Which no longer have staging_id
...
BEGIN
    ...
    SELECT job_id, txn_no, raw_row, critical_array, warning_array
    FROM evaluated_data
...
BEGIN
    ...
    SELECT job_id, cif, raw_row, critical_array, warning_array
    FROM evaluated_data
    WHERE cardinality(critical_array) > 0 OR cardinality(warning_array) > 0
        ),
